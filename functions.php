...
function getCountries($mysql_link){
	$sql = "SELECT * FROM mes_countries";
	$result = mysqli_query($mysql_link, $sql);
	return getResult($result);
}

function getPlaces($mysql_link, $type = NULL, $row_name = NULL, $id = 1){
	if($type == NULL){
		$type = 'mes_regions';
	}
	...
	switch($type){
		case 'region':
			$sql = "SELECT * FROM mes_regions WHERE $row_name = $id";
			$region = getResult(mysqli_query($mysql_link, $sql));

			$sql = "SELECT * FROM mes_cities WHERE $row_name = $id and region_id is NULL";
			$city = getResult(mysqli_query($mysql_link, $sql));
			
			$result = array('region' => $region, 'city' => $city);
			break;
		case 'city':
			$sql = "SELECT * FROM mes_cities WHERE $row_name = $id ORDER BY name_city ASC";
			$result = getResult(mysqli_query($mysql_link, $sql));
			break;
	}
...
//function getTowns($mysql_link, $type = NULL, $row_name = NULL, $id = 1){
//	if($type == NULL){
//		$type = 'mes_cities';
//	}
//
...
//	}
//
//	$sql = "SELECT * FROM $table WHERE $row_name = $id";
//	$result = mysqli_query($mysql_link, $sql);
//
//	return getResult($result);
//}
